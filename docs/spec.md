...
anywhere else. The three directories are kept separate because they're
different package conventions (§4 vs §4a vs §4b) merged into one flat
scope, not because the scope itself distinguishes them. The scope's
`extra` attributes make four builders available to every package in the
scope via `callPackage`:

- `mkPulumiPackage` — this repository's `pkgs/mk-pulumi-package.nix`
  (see below).
- `mkComponentPackage` — this repository's `pkgs/mk-component-package.nix`
  (§4b).
